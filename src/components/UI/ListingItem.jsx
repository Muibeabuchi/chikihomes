      ...
      <Link to={`/category/${category}/${id}`} className=' bg-white flex flex-col  justify-center shadow-md hover:shadow-xl rounded-md overflow-hidden transition-shadow ease-in-out duration '>
        ...
        <div className="w-full p-[10px] ">
          <div className="flex items-center space-x-1">
            ...
            <p className='font-semibold text-sm mb-[2px] text-gray-600 truncate'>{address}</p>
          </div>
          <p className='font-semibold text-xl m-0 truncate'>{name}</p>
          <p className='text-[#457b9d] mt-2 font-semibold'>GH₵{offer ? discountedprice.toString().replace(/\B(?=(\d{3})+(?!\d))/g,',') : regularprice.toString().replace(/\B(?=(\d{3})+(?!\d))/g,',')} {type==='rent' && ' /month'}</p>
          <div className="flex items-center mt-[10px] space-x-3 ">
            {/* <div className=""> */}
              <p className='font-bold text-sm'>{bedrooms > 1 ? `${bedrooms} Beds` : '1 Bed'}</p>
              <p className='font-bold text-sm'>{bathrooms > 1 ? `${bathrooms} Baths` : '1 Bath'}</p>
            {/* </div> */}
          </div>
        </div>
      </Link>
